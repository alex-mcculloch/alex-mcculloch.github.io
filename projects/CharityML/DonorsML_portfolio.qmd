...
We split the data into training and testing sets, using 80% of the data for training and 20% for evaluation. This ensures the model is assessed on data it has not seen before.
...
In this dataset, the majority (75%) of individuals earn less than \$50,000, meaning a model could achieve relatively high accuracy simply by predicting the most common outcome. To account for this, we use Cohen’s Kappa, a statistic that measures how much better the model performs compared to random chance. Specifically, it measures agreement between two raters or classifiers correcting for agreement that would happen by chance. It ranges from -1 (complete disagreement) to 1 (perfect agreement) with 0 indicating agreement no better than random chance.
...
Before evaluating the value of the model, we need as baseline for comparison.

```{r}
# baseline rate
mean(test$income==1) #25%
# if contact at random, only 25% earn more than 50K
```

Only about 25% of individuals in the dataset earn \$50K or more. This means that if a nonprofit were to contact donors at random, roughly one in four would fall into the higher-income category.

### Measuring Lift

Lift compares the model's targeting performance against random outreach. It informs us of how much more likely we are to reach a high-income earner by using the model. We can summarize outcome by donor tier below.
...
Earners in the high priority tier are 3.5 times more likely to earn \$50K or more compared to random selection. Even the medium tier is 2.5 times more likely to include higher-income individuals.
...
To see this model in action, I have created a Shiny web application that can be found [here](https://4blh4z-alex-mcculloch.shinyapps.io/charityml/). You can calculate an individual's probability of being a \$50K+ earner by entering information in the right panel. You can also see an example of what it would look like if you were to upload a file containing potential donor information by selecting the "Load Sample Donor Data" button.
